...
public class Missile : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // Rotera missilen runt sin egen axel
        missile.Rotate(new Vector3(0f, 0f, rotation));

        // Flytta missilen framåt
        rb.linearVelocity = transform.forward * speed;

        // Rikta in missilen mot spelaren
        Quaternion missileTargetRotation = Quaternion.LookRotation(player.position - transform.position);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.collider.CompareTag("Player"))
        {
            ...
            // Skapa explosion
            GameObject explosion = Instantiate(missileExplosionPrefab, collision.transform.position, Quaternion.identity);
        ...
        {
            // Förstör missilen om den träffar något annat än spelaren
            GameObject explosion = Instantiate(missileExplosionPrefab, transform.position, Quaternion.identity);
            Destroy(explosion, 4f);
            Destroy(gameObject);
        }
    }
