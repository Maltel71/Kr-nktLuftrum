...
public class CameraShake : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            instance = this;
            DontDestroyOnLoad(gameObject);  // Lägg till för att behålla mellan scener
            SetupCamera();
        }
        else
        {
    ...
    {
        StartCoroutine(Skaka(3.0f, 0.4f));
    }
    ...
    {
        StartCoroutine(Skaka(0.2f, 0.3f));
    }
    ...
    private IEnumerator Skaka(float intensitet, float varaktighet)
    {
        if (cameraTransform == null)
        {
            Debug.LogError("Kan inte skaka - cameraTransform är null!");
            yield break;
        }
